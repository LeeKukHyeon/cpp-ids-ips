...
PacketInfo Parser::parse(const u_char* data, int len) {
	PacketInfo pkt;
	if (len < (int)sizeof(struct ether_header)) return pkt;
	...
	int transport_len = len - sizeof(struct ether_header) - ip_header_len;

	if (pkt.protocol == IPPROTO_TCP && transport_len >= (int)sizeof(struct tcphdr)) {
	...
	else if (pkt.protocol == IPPROTO_UDP && transport_len >= (int)sizeof(struct udphdr)) {
